...
import { EXAM_CONFIG } from "@/lib/ap-computer-science-principles/config";

describe("AP Computer Science Principles Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM179");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("ap-computer-science-principles");
  });
